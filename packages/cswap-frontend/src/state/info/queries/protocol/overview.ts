...
const formatCswapFactoryResponse = (rawCswapFactory?: CswapFactory) => {
  if (rawCswapFactory) {
    return {
      totalTransactions: parseFloat(rawCswapFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawCswapFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawCswapFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatCswapFactoryResponse(data?.cswapFactories?.[0])

      // When CSWAP is first launched, both 24 and 48 hr will be null
      const overviewData24Formatted = formatCswapFactoryResponse(data24?.cswapFactories?.[0])
      const overviewData24 = overviewData24Formatted || overviewData
      const overviewData48Formatted = formatCswapFactoryResponse(data48?.cswapFactories?.[0]) // FIXME hard code no data for 48h
