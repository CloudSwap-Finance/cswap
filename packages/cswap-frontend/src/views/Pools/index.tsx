...
import { Heading, Flex, Image, Text } from 'cswap-uikit'
import orderBy from 'lodash/orderBy'
import partition from 'lodash/partition'
import { useTranslation } from 'contexts/Localization'
import useIntersectionObserver from 'hooks/useIntersectionObserver'
import { useFetchPublicPoolsData, usePools, useFetchUserPools, useFetchCswapVault, useCswapVault } from 'state/pools/hooks'
...
import CswapVaultCard from './components/CswapVaultCard'
import PoolTabButtons from './components/PoolTabButtons'
import BountyCard from './components/BountyCard'
import HelpButton from './components/HelpButton'
import PoolsTable from './components/PoolsTable/PoolsTable'
import { getAprData, getCswapVaultEarnings } from './helpers'
      ...
      case 'totalStaked':
        return orderBy(
          ...
          (pool: DeserializedPool) => {
            ...
            if (pool.isAutoVault) {
              if (totalCswapInVault.isFinite()) {
                totalStaked = +formatUnits(
                  ethers.BigNumber.from(totalCswapInVault.toString()),
                  pool.stakingToken.decimals,
                )
              }
            } else if (pool.sousId === 0) {
              if (pool.totalStaked?.isFinite() && totalCswapInVault.isFinite()) {
                const manualCswapTotalMinusAutoVault = ethers.BigNumber.from(pool.totalStaked.toString()).sub(
                  totalCswapInVault.toString(),
                )
                totalStaked = +formatUnits(manualCswapTotalMinusAutoVault, pool.stakingToken.decimals)
              }
