...
import { Flex, TooltipText, IconButton, useModal, CalculateIcon, Skeleton, useTooltip } from 'cswap-uikit'
...
const AprRow: React.FC<AprRowProps> = ({ pool, stakedBalance, performanceFee = 0 }) => {
  ...
  const tooltipContent = isBlindMode() ? t('Amount of CSWAP rewards allocated to the Glitter mine participants issued with each Cronos block (~5-6s/block). We will convert this value to APY/APR value after Pioneer Farming Mode is completed.') : (isAutoVault
  ...
  const cswapFarm = useFarmFromPid(0)
  ...
  return (
    isBlindMode() ? <Flex alignItems="center" justifyContent="space-between">
      ...
      <Flex alignItems="center">
        {cswapFarm && cswapFarm.poolWeight ? `${(CSWAP_PER_BLOCK * cswapFarm.poolWeight.toNumber()).toFixed(2)}/ Block` : ''}
      </Flex>
