...
import { Flex, Text, Skeleton, Link, Button, ArrowForwardIcon } from 'cswap-uikit'
...
const LotteryCardContent = () => {
  ...
  const cswapPriceUsdcAsString = usePriceCswapUsdc().toString()

  const cswapPrizesText = t('%cakePrizeInUsd% in CSWAP prizes this round', { cakePrizeInUsd: cswapPriceUsdcAsString })
  const [pretext, prizesThisRound] = cswapPrizesText.split(cswapPriceUsdcAsString)

  const cswapPriceUsdc = useMemo(() => {
    return new BigNumber(cswapPriceUsdcAsString)
  }, [cswapPriceUsdcAsString])
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      ...
      const prizeInUsdc = cswapPriceUsdc.times(amountCollectedInCake)
  ...
  }, [lotteryId, slowRefresh, setCurrentLotteryPrize, cswapPriceUsdc])
