...
import { Flex, Heading, Text, Button, Link, useMatchBreakpoints } from 'cswap-uikit'
import { useTranslation } from 'contexts/Localization'
import { isCountdown } from 'utils'

import { StyledBigHeading } from '../StyledHeadings'
import { cswapTokenIntroData } from './data'
...
const CswapTokenIntro = () => {
  const { t } = useTranslation()

  const { headingText, items } = cswapTokenIntroData
              ...
              >
                <Flex flexDirection={['column', 'column', 'column', 'row']} mb="10px">
                  ...
                  <Text style={{ alignSelf: 'center', fontWeight: 'bold' }} height="30px" lineHeight="34px" width="100%">
                    <img
                      src="/images/home/cswap-info/aquaVector.svg"
                      alt="star1"
                      height="18px"
                      width="18px"
                      style={{
          ...
          <Flex justifyContent={isDesktop ? 'unset' : 'center'}>
            {
              isCountdown() ? (
                <>
                  ...
                  <>
                    <Link external href="https://docs.cswap.finance/litepaper">
                      <StyledButton mr={10}>{t('White Paper')}</StyledButton>
                    </Link>
                    <Link external href="https://docs.cswap.finance/litepaper/token-economics">
                      <StyledButton variant="secondary">{t('CSWAP Tokenomics')}</StyledButton>
                    </Link>
                  </>
              )
            }
...
export default CswapTokenIntro
