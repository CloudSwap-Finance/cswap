...
import { Box, Button, Flex, InjectedModalProps, LinkExternal, Message, Skeleton, Text } from 'cswap-uikit'
...
const WalletInfo: React.FC<WalletInfoProps> = ({ hasLowCROBalance, onDismiss }) => {
  ...
  const { balance: cswapBalance, fetchStatus: cswapFetchStatus } = useTokenBalance(tokens.cswap.address)
  ...
  return (
    <>
      ...
      <Flex alignItems="center" justifyContent="space-between" mb="24px" style={balanceRowStyles}>
        <Text color="textSubtle">{t('CSWAP Balance')}</Text>
        {cswapFetchStatus !== FetchStatus.SUCCESS ? (
          <Skeleton height="22px" width="60px" />
        ) : (
          <Text>{getFullDisplayBalance(cswapBalance, 18, 3)}</Text>
        )}
      </Flex>
