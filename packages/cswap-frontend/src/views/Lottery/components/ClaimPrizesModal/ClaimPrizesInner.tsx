...
import { Flex, Button, Text, AutoRenewIcon, PresentWonIcon } from 'cswap-uikit'
...
const ClaimInnerContainer: React.FC<ClaimInnerProps> = ({ onSuccess, roundsToClaim }) => {
  ...
  const cswapReward = activeClaimData.cakeTotal
  const dollarReward = cswapReward.times(cswapPriceUsdc)
  const rewardAsBalance = getBalanceAmount(cswapReward).toNumber()
