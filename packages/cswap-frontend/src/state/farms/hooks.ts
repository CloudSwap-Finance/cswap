...
export const usePriceCswapUsdc = (): BigNumber => {
  // const cswapCroFarm = useFarmFromPid(3) // FIXME pid for CSWAP/WCRO
  const cswapCroFarm = useFarmFromLpPair(tokens.cswap, tokens.wcro) // FIXME pid for CSWAP/WCRO

  const cswapPriceUsdcAsString = cswapCroFarm.tokenPriceUsdc

  const cswapPriceUsdc = useMemo(() => {
    return new BigNumber(cswapPriceUsdcAsString)
  }, [cswapPriceUsdcAsString])

  return cswapPriceUsdc
  // const farms = useSelector((state: State) => state.farms)
  // const prices = getTokenPricesFromFarm(farms.data)
  // return prices[serializeTokens().cswap.address] ?? new BigNumber(0);
}
