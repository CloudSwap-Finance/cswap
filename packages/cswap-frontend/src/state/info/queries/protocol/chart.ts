...
import { CSWAP_FINANCE_START } from 'config/constants/info'
import { ChartEntry } from 'state/info/types'
import { CswapDayDatasResponse } from '../types'
import { fetchChartData, mapDayData } from '../helpers'

/**
 * Data for displaying Liquidity and Volume charts on Overview page
 */
const CSWAP_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    cswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (skip: number): Promise<{ data?: ChartEntry[]; error: boolean }> => {
  try {
    const { cswapDayDatas } = await request<CswapDayDatasResponse>(INFO_CLIENT, CSWAP_DAY_DATAS, {
      startTime: CSWAP_FINANCE_START,
      skip,
    })
    const data = cswapDayDatas.map(mapDayData)
