...
import { Card, CardBody, Flex, Skeleton, Text } from 'cswap-uikit'
...
const PlatformYieldCard = () => {
  ...
  useEffect(() => {
    const fetchHarvestedData = async () => {
      ...
      if (harvested?.totalClaimAmount || harvested?.totalCSWAPVaultProfit) {
        setHarvestedSum(Number(harvested.totalClaimAmount) + Number(harvested.totalCSWAPVaultProfit))
      }
    }
    if (account) {
      fetchHarvestedData()
  ...
  const totalPlatformYield = (getBalanceNumber(new BigNumber(harvestedSum), tokens.cswap.decimals) + farmEarningsSum) * cswapPriceUsdc.toNumber()
