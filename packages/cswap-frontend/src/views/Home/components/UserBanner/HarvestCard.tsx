...
import { AutoRenewIcon, Button, Card, CardBody, Flex, Skeleton, Text, Link, ArrowForwardIcon } from 'cswap-uikit'
...
const HarvestCard = () => {
  ...
  // const cswapPriceUsdc = new BigNumber(0) // XXX debug hard code for landing banner
  const earningsUsdc = new BigNumber(farmEarningsSum).multipliedBy(cswapPriceUsdc)
  ...
  const hasCswapPoolToCollect = numTotalToCollect - numFarmsToCollect > 0

  const earningsText = getEarningsText(numFarmsToCollect, hasCswapPoolToCollect, earningsUsdc, t)
  ...
  const harvestAllFarms = useCallback(async () => {
    ...
    for (const farmWithBalance of farmsWithStakedBalance) {
      try {
        ...
        toastSuccess(
          `${t('Harvested')}!`,
          t('Your %symbol% earnings have been sent to your wallet!', { symbol: 'CSWAP' }),
        )
      } catch (error) {
