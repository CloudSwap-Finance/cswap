...
import { ChainId, WETH } from 'cswap-sdk'
...
export const useCswap = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getCswapContract(library.getSigner()), [library])
}
...
export const useCswapVaultContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getCswapVaultContract(library.getSigner()), [library])
}
