...
import { Image, Heading, RowType, Toggle, Text, Button, ArrowForwardIcon, Flex } from 'cswap-uikit'
import { ChainId } from 'cswap-sdk'
import styled from 'styled-components'
import FlexLayout from 'components/Layout/Flex'
import Page from 'components/Layout/Page'
import { useFarms, usePollFarmsWithUserData, usePriceCswapUsdc } from 'state/farms/hooks'
...
export const getDisplayApr = (cswapRewardsApr?: number, lpRewardsApr?: number) => {
  if (cswapRewardsApr && lpRewardsApr) {
    return (cswapRewardsApr + lpRewardsApr).toLocaleString('en-US', { maximumFractionDigits: 2 })
  }
  if (cswapRewardsApr) {
    return cswapRewardsApr.toLocaleString('en-US', { maximumFractionDigits: 2 })
  }
  return null
}
...
const Farms: React.FC = () => {
  ...
  const farmsList = useCallback(
    (farmsToDisplay: DeserializedFarm[]): FarmWithStakedValue[] => {
      let farmsToDisplayWithAPR: FarmWithStakedValue[] = farmsToDisplay.map((farm) => {
        ...
        const { cswapRewardsApr, lpRewardsApr } = isActive
          ? getFarmApr(new BigNumber(farm.poolWeight), cswapPrice, totalLiquidity, getAddress(farm.lpAddresses))
          : { cswapRewardsApr: 0, lpRewardsApr: 0 }
  ...
  const rowData = chosenFarmsMemoized.map((farm) => {
    ...
    // const lpLabel = farm.lpSymbol && farm.lpSymbol.split(' ')[0].toUpperCase().replace('CSWAP', '') // FIXME replace ??
