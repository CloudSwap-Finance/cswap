...
import { Modal, Text, Flex, BalanceInput, Box, Button, PancakeRoundIcon } from 'cswap-uikit'
...
import { useCswap, useFarmAuctionContract } from 'hooks/useContract'
...
const PlaceBidModal: React.FC<PlaceBidModalProps> = ({
  ...
  useEffect(() => {
    if (userNotEnoughCake) {
      setErrorText(t('Insufficient CSWAP balance'))
    } else if (!isMoreThanInitialBidAmount && isFirstBid) {
      setErrorText(t('First bid must be %initialBidAmount% CSWAP or more.', { initialBidAmount }))
    ...
    <StyledModal title={t('Place a Bid')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <InnerContent>
        ...
        <BalanceInput
          ...
          currencyValue={
            cswapPriceUsdc.gt(0) &&
            `~${bid ? cswapPriceUsdc.times(new BigNumber(bid)).toNumber().toLocaleString() : '0.00'} USD`
          }
        />
